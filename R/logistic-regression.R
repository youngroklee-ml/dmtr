...
loglik_ordinal_logistic_regression <- function(.betas, .x, .y,
  ...
  if (.type == "cumulative") {
    ...
    score_mat <- cbind(
      exp(t(apply(.x %*% .betas, 1, function(x) rev(cumsum(rev(x)))))),
      rep(1, nrow(.x))
    )
    denominator <- rowSums(score_mat)
    p <- score_mat / denominator
...
#' TO DO: 초기 파라미터 값에 따라 모형 추정 결과가 다른 문제 해결
#'
#' @param .data 관측 데이터 프레임.
#' @param .group_var 범주변수.
#' @param ... 범주 분류에 사용될 변수.
...
#' )
#' fit_ordinal_logistic_regression(
#'   telconnection, y, N, L, .type = "adjacent"
#' )
#'
#' @export
fit_ordinal_logistic_regression <- function(
  .data, .group_var, ..., .type = "cumulative",
  .control = list(fnscale = -1, maxit = 10000)
) {
  .group_var <- rlang::enquo(.group_var)
  .variables <- rlang::enquos(...)
  ...
  if (n_levels < 3L) stop(".group_var needs to have three or more levels.")

  n_betas <- length(.variables) + 1L
  betas <- c(
    sort(stats::runif(n = n_levels - 1L)),
    stats::runif(n = length(.variables))
  )
