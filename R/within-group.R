...
#' group.
#'
#' 이 함수는 데이터 프레임 \code{.data} 내의 범주변수 \code{.group_var}의 범주별로 \code{...}에 주어진
...
#' variable and columns \code{...} that represent group mean value of
#' \code{...} in the original data \code{.data}.
#'
#' @param .data 관측 데이터 프레임. A raw data frame.
#' @param .group_var 범주변수. A variable to group by.
#' @param ... 평균벡터에 포함될 변수. One or more variables separated by commas. Each variable needs to
...
#'   and has a vector of mean of \code{...}.
#'
#' @examples
#' data(binaryclass2, package = "dmtr")
#' group_mean(binaryclass2, class, x1, x2)
#'
#' @export
...
#' as group variable and columns \code{...} that represent group mean value of
#' \code{...} in the original data \code{.data}.
#'
#' @param .data 관측 데이터 프레임. A raw data frame.
#' @param .group_var 범주변수. A variable to group by.
#' @param ... 분산-공분산 행렬에 포함될 변수. One or more variables separated by commas. Each
...
#'
#' @export
group_variance <- function(.data, .group_var, ...) {
  .group_var <- rlang::enquo(.group_var)
...
#' @param .data 관측 데이터 프레임. A raw data frame.
#' @param .group_var 범주변수. A variable to group by.
#' @param ... 관심 변수. One or more variables separated by commas. Each variable
#'   needs to be a numeric column of \code{.data}.
#' @return 리스트 형태로, 각 리스트의 원소는 각 범주에 대한 리스트를 지닌다.
#'
#' @examples
#' data(binaryclass2, package = "dmtr")
#' group_summary(binaryclass2, class, x1, x2)
#'
#' @export
...
#'
#' @export
pooled_variance <- function(.data, .group_var, ...) {
  .group_var <- rlang::enquo(.group_var)

  summ <- group_summary(.data, !!.group_var, ...)
