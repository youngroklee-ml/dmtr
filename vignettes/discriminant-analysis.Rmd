...
```{r}
predict_da(f, .new_data = binaryclass2, x1, x2, 
  .include_score = TRUE,
...
```{r}
prediction_df <- predict_da(f, .new_data = binaryclass2, x1, x2, 
  .include_score = TRUE,
  .include_posterior = TRUE,
  .include_class = TRUE)
...
```{r}
f <- ld_fun(iris90, class, x1, x2, x3, x4)
predict_df <- predict_da(f, .new_data = iris90, x1, x2, x3, x4, 
  .include_posterior = TRUE)
...
```{r}
f <- qd_fun(iris90, class, x1, x2, x3, x4)
predict_df <- predict_da(f, .new_data = iris90, x1, x2, x3, x4, 
  .include_posterior = TRUE)
